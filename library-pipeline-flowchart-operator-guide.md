...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0 — Orientation                  | Read `PAGE_SCHEMA.md`, `VOCABULARY_GUIDE.md`, `index.md`, and `log.md`; validate the configured lanes in `_meta/config/domain.json`; ensure `.gitignore` protects local secrets and transient files.                                                                                                                                                   | Ready to start.              | Operator context; no phase gate.                                                                                                                                                                                                        |
...
| 2.1 + 2.2 — Boundaries and units | `python3 _meta/scripts/library_phase2_chapters.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                      | `READY_FOR_2.3`              | `chapter-boundaries.json` when manual TOC boundaries are needed, `chapters/*.md`, `manifest.json`, `phase-2.1.json`, and `phase-2.2.json`. Fixed-size fallback chunks fail the canonical gate; the one runner writes both gates.        |
| 2.3 — Block IDs                  | `python3 _meta/scripts/library_phase23_blocks.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                       | `READY_FOR_2.4`              | Same-slug inline block IDs, `block_annotator-report.json`, and `phase-2.3.json`.                                                                                                                                                        |
| 2.4 — Images                     | `python3 _meta/scripts/library_phase24_images.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                       | `READY_FOR_3.0`              | Rebased `chapters/images/`, `image-refs-report.json`, and `phase-2.4.json`. Remote and data URLs are rejected.                                                                                                                          |
| 3.0 — Vision enrichment          | `python3 _meta/scripts/library_phase30_vision.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                       | `READY_FOR_3.1`              | One `orchestrator-vision-enrichment.md` per unit, `vision-enrichment-report.json`, and `phase-3.0.json`. Unresolved `VISION_*_NEEDED` markers fail.                                                                                     |
| 3.1 — Source index               | `python3 _meta/scripts/library_phase31_source_index.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                 | `READY_FOR_3.2`              | One `orchestrator-source-index.md` per unit, `source-index-report.json`, and `phase-3.1.json`. Every current same-slug block is indexed exactly once.                                                                                   |
| 3.2 — Size split                 | `python3 _meta/scripts/library_phase32_size_split.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                   | `READY_FOR_3.3`              | Active units no larger than 2,000 lines, `size-split-report.json`, refreshed vision/index artifacts, and `phase-3.2.json`. Splits have zero overlap.                                                                                    |
| 3.3 — Specialist dispatch        | Run `python3 _meta/scripts/library_phase33_dispatch.py --slug "$SLUG" --prepare`; use the generated plan to launch native specialist tasks; write `_meta/extractions/$SLUG/dispatch-result.json`; then run `python3 _meta/scripts/library_phase33_dispatch.py --slug "$SLUG" --record --dispatch-result _meta/extractions/$SLUG/dispatch-result.json`. | `READY_FOR_3.4`              | `specialist-dispatch-plan.json`, `specialist-dispatch-report.json`, one markdown and schema draft per unit × configured lane, and `phase-3.3.json`. Recording requires real runtime, model, runtime task ID, and job ID for every task. |
| 3.4 — Specialist verification    | `python3 _meta/scripts/library_phase34_verify.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                       | `READY_FOR_3.5`              | `specialist-verification.json`, `schema-validation-report.json`, machine-owned `_validation_passed`, `pipeline-run-manifest.json`, and `phase-3.4.json`. It validates the recorded lane outputs only; presentations do not exist yet.   |
| 3.5 — Team presentation          | `python3 _meta/scripts/library_phase35_presentations.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                | `READY_FOR_4`                | One `team-<unit_id>-presentation.md` per current unit, `presentation-report.json`, advisory presentation-evidence-balance audits, and `phase-3.5.json`.                                                                                 |
| 4a — Candidate preparation       | `python3 _meta/scripts/library_phase4_merge_score.py --slug "$SLUG" --prepare`                                                                                                                                                                                                                                                                         | `AWAITING_USER_CONFIRMATION` | Scored and clean candidate sets, candidate and rationale packets, block-ID validation, and `phase-4.json` with status `AWAITING_USER_CONFIRMATION`.                                                                                     |
| 4b — Human confirmation          | A human writes `_meta/extractions/$SLUG/phase4-user-selection.json` with non-empty `confirmed_slugs`, then run `python3 _meta/scripts/library_phase4_merge_score.py --slug "$SLUG" --confirm --selection _meta/extractions/$SLUG/phase4-user-selection.json`.                                                                                          | `READY_FOR_5`                | `phase4-confirmation.json`, `master-confirmed.json`, and a `PASS` Phase 4 gate.                                                                                                                                                         |
| 5 — Page build                   | `python3 _meta/scripts/library_phase5_pages.py --slug "$SLUG"`                                                                                                                                                                                                                                                                                         | `READY_FOR_POST`             | `concepts/*.md`, `page-build-report.json`, `phase-5.json`, plus updated `index.md` and `log.md`. Pages are written only for confirmed concepts and derive prose/evidence from validated Phase 3.5 presentations.                        |
| Post — Finalization              | `python3 _meta/scripts/library_audit.py --slug "$SLUG" --wiki . --report _meta/reports/audit-$SLUG.json [--ack CHECK_ID,...]`                                                                                                                                                                                                                          | `DONE`                       | Lexical-overlap and image-coverage grounding reports, final audit JSON, `phase-post.json`, and the `DONE` state. `library_audit.py` runs both grounding QA and audit; there is no separate post command.                                |
