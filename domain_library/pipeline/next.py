...
from _meta.scripts.pipeline_common import validate_slug
...
NEXT = {
    "READY_FOR_2": "domain-library run library_phase2_chapters --slug {slug}",
    "READY_FOR_2.1": "domain-library run library_phase2_chapters --slug {slug}",
    "READY_FOR_2.2": "domain-library run library_phase2_chapters --slug {slug}",
    "READY_FOR_2.3": "domain-library run library_phase23_blocks --slug {slug}",
    "READY_FOR_2.4": "domain-library run library_phase24_images --slug {slug}",
    "READY_FOR_3.0": "domain-library run library_phase30_vision --slug {slug}",
    "READY_FOR_3.1": "domain-library run library_phase31_source_index --slug {slug}",
    "READY_FOR_3.2": "domain-library run library_phase32_size_split --slug {slug}",
    "READY_FOR_3.3": (
        ...
        "  then: domain-library run library_phase33_dispatch --slug {slug} --record "
        "--dispatch-result _meta/extractions/{slug}/dispatch-result.json"
    ),
    "READY_FOR_3.4": "domain-library run library_phase34_verify --slug {slug}",
    "READY_FOR_3.5": "domain-library run library_phase35_presentations --slug {slug}",
    "READY_FOR_4": "domain-library run library_phase4_merge_score --slug {slug} --prepare",
    "AWAITING_USER_CONFIRMATION": (
        ...
        "     as {{\"confirmed_slugs\": [...]}}.\n"
        "  3. domain-library run library_phase4_merge_score --slug {slug} --confirm "
        "--selection _meta/extractions/{slug}/phase4-user-selection.json"
    ),
    "READY_FOR_5": "domain-library run library_phase5_pages --slug {slug}",
    "READY_FOR_POST": (
        "domain-library run library_audit --slug {slug} --wiki . "
...
def inspect(wiki: Path, slug: str) -> dict:
    ...
    if not state_file.exists():
        ...
        out["next"] = (
            ...
            "  Start: domain-library run library_phase1_ocr --slug {slug} "
    ...
    if status in NEXT:
        ...
        out["next"] = (
            ...
            "Never edit gate/state files by hand."
        )
