...
def extract_text(
    ...
    Args:
        ...
        **options: Additional API options:
            ...
            - start_page_id: optional int - First PDF page to process
            - end_page_id: optional int - Last PDF page to process; omitted means full document
    ...
    # Build request payload. Do not silently cap PDFs at page 2: include page-range
    # fields only when the caller specifies them, otherwise let the API process
    ...
    if options.get("start_page_id") is not None:
...
Configuration:
  Copy the repository root .env.example to .env and set ZHIPU_API_KEY.
        """,
    )
    ...
    parser.add_argument(
        "--start-page-id", type=int, default=None, help="First PDF page to process (optional)"
    )
    parser.add_argument(
        "--end-page-id", type=int, default=None, help="Last PDF page to process (optional; omit for full document)"
    )
    parser.add_argument(
        "--return-crop-images", action="store_true", help="Ask GLM-OCR API to return cropped image/table/chart assets"
    )
    parser.add_argument(
        "--need-layout-visualization", action="store_true", help="Ask GLM-OCR API to return layout visualization assets"
    )
    ...
    result = extract_text(
        ...
        return_crop_images=args.return_crop_images,
        need_layout_visualization=args.need_layout_visualization,
    )
