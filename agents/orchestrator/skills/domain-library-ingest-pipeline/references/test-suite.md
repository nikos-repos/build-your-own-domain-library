...
| `test_phase2_recovers_from_detection_failure_and_failed_state` | detection failure without boundaries leaves state `READY_FOR_2.1`; a genuine failure keeps prior gate records and is rerunnable from `FAILED`                                                            |
...
| `test_cli_run_executes_phase4_and_phase5_runners`            | `domain-library run` (`python -m`) can execute the Phase 4 and Phase 5 runners end to end — guards against bare sibling imports that only work via direct paths                                            |
