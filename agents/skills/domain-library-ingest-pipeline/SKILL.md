...
## Public-release configuration checklist

Before publishing this skill with a repository, confirm that:

- Script names in the command table match the public repository.
- Private usernames, machine paths, local runtime names, and organization-specific acronyms have been removed or documented as placeholders.
- `PAGE_SCHEMA.md` and `VOCABULARY_GUIDE.md` are included, or references are updated to the local filenames.
- Example slugs are generic and not tied to a private project.
- Required support skills or external services are documented with installation/setup instructions.
- Provider-specific OCR limits are either documented or parameterized.
- `.gitignore` excludes private raw files, credentials, local runtime caches, temporary chunks, test outputs, and system artifacts.
- Public sample data is licensed for redistribution.
