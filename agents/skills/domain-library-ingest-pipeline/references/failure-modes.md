...
Direct one-shot LLM extraction may be useful as a diagnostic but is not a pipeline fallback. It is often structurally valid but content-thin. Any such output must be marked `thin_output: true` and fail normal quality gates unless explicitly accepted by the user.
